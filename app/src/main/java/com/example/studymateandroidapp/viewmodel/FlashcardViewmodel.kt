...
class FlashcardViewmodel(
    ...
    fun completeFlashcardSession(examId: Long, correctCount: Int, totalCount: Int) {
        viewModelScope.launch {
            val performance = if (totalCount > 0) correctCount.toFloat() / totalCount else 0f
            studyProgressRepository.getProgressByExamId(examId).take(1).collect { current ->
                ...
                // Update flashcard mastery (rolling average)
                val newFlashcardMastery = if (progress.flashcardMastery == 0f) performance else (progress.flashcardMastery + performance) / 2f
                
                // Mastery Logic: 70% flashcards, 30% time (target 5 hours)
                val timePoints = (progress.totalStudyTime.toFloat() / (5 * 60 * 60 * 1000)).coerceAtMost(1f) * 0.3f
